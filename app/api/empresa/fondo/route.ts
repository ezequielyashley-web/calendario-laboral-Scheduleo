...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (file.size > 4 * 1024 * 1024) return NextResponse.json({ error: "La imagen no puede superar 4MB" }, { status: 400 })
    ...
    const nombreArchivo = `empresa-001-fondo-${Date.now()}.${ext}`
    ...
    if (!res.ok) {
      ...
      console.error("Error subiendo fondo a Supabase Storage:", errorText)
