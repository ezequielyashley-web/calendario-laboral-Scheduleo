...
function InspeccionContent() {
  ...
  const [alertas, setAlertas] = useState<any>({ alertas: [], totalEmpleados: 0, totalAlertas: 0 })
  ...
  const SECCIONES = [
    ...
    { key: "alertas", label: "Alertas legales", icon: "⚠️" },
  ]
  ...
  const cambiarSeccion = (s) => {
    setSeccion(s)
    if (s === "alertas") { cargarAlertas() } else { cargarDatos(s) }
  }
  ...
  const exportarPDF = async () => {
    ...
    if (seccion === "alertas") {
      doc.text(`${alertas.totalAlertas} alertas · ${alertas.totalEmpleados} empleados analizados`, 14, 46)
      columns = [
        ...
        { header: "Exceso (h)", dataKey: "exceso" },
        { header: "Gravedad", dataKey: "gravedad" },
      ]
      rows = alertas.alertas.map((a: any) => ({
        num: a.numeroEmpleado,
        nombre: a.nombre,
        apellidos: a.apellidos,
        semana: a.semana,
        horas: a.horasTrabajadas + "h",
        exceso: "+" + a.horasExceso + "h",
        gravedad: a.gravedad,
      }))
    } else if (seccion === "fichajes") {
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff" }}>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: 24, display: "grid", gridTemplateColumns: "220px 1fr", gap: 20 }}>
        ...
        <div>
          <div style={{ background: "#fff", borderRadius: 14, padding: 12, marginBottom: 12 }}>
            ...
            {SECCIONES.map(s => (
              <button key={s.key} onClick={() => cambiarSeccion(s.key)}
                ...
                {s.key === "alertas" && alertas.totalAlertas > 0 && (
                  <span style={{ background: "#dc2626", color: "#fff", borderRadius: 20, fontSize: 10, fontWeight: 700, padding: "1px 6px", marginLeft: "auto" }}>{alertas.totalAlertas}</span>
                )}
              </button>
            ))}
          </div>

          {seccion !== "modificaciones" && seccion !== "empleados" && seccion !== "alertas" && (
          ...
          {seccion === "alertas" && (
            <div style={{ background: "#fff", borderRadius: 14, padding: 16 }}>
              <div style={{ fontSize: 11, color: "#a0aec0", fontWeight: 600, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>Período análisis</div>
              <div style={{ marginBottom: 10 }}>
                <label style={{ display: "block", fontSize: 11, color: "#a0aec0", marginBottom: 4 }}>Desde</label>
                <input type="date" value={filtros.desde} onChange={e => setFiltros(p => ({ ...p, desde: e.target.value }))}
                  style={{ width: "100%", padding: "7px 10px", border: "1px solid #e8eaf0", borderRadius: 7, fontSize: 13, boxSizing: "border-box" }} />
              ...
              <button onClick={cargarAlertas}
                style={{ width: "100%", background: "#dc2626", color: "#fff", border: "none", borderRadius: 8, padding: "9px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
                Analizar período
              </button>
            </div>
          )}
        ...
        <div>
          {seccion === "alertas" ? (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 16 }}>
                <div style={{ background: alertas.totalAlertas > 0 ? "#fee2e2" : "#d1fae5", borderRadius: 12, padding: "16px 20px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: alertas.totalAlertas > 0 ? "#991b1b" : "#065f46", fontWeight: 500, marginBottom: 4 }}>Total alertas</div>
                  <div style={{ fontSize: 28, fontWeight: 700, color: alertas.totalAlertas > 0 ? "#dc2626" : "#059669" }}>{alertas.totalAlertas}</div>
                </div>
                <div style={{ background: "#f0f4ff", borderRadius: 12, padding: "16px 20px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#6366f1", fontWeight: 500, marginBottom: 4 }}>Empleados analizados</div>
                  <div style={{ fontSize: 28, fontWeight: 700, color: "#6366f1" }}>{alertas.totalEmpleados}</div>
                </div>
                <div style={{ background: "#fef3c7", borderRadius: 12, padding: "16px 20px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#92400e", fontWeight: 500, marginBottom: 4 }}>Jornada máxima legal</div>
                  <div style={{ fontSize: 28, fontWeight: 700, color: "#d97706" }}>37.5h</div>
                </div>
              </div>

              {loadingAlertas ? (
                <div style={{ background: "#fff", borderRadius: 14, padding: 40, textAlign: "center", color: "#a0aec0" }}>Analizando jornadas...</div>
              ) : alertas.totalAlertas === 0 ? (
                <div style={{ background: "#d1fae5", borderRadius: 14, padding: 40, textAlign: "center" }}>
                  <div style={{ fontSize: 32, marginBottom: 12 }}>✅</div>
                  <div style={{ fontSize: 15, fontWeight: 600, color: "#065f46" }}>Sin infracciones detectadas</div>
                  <div style={{ fontSize: 13, color: "#6ee7b7", marginTop: 4 }}>Todos los empleados cumplen la jornada máxima de 37.5h semanales</div>
                </div>
              ) : (
                <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden" }}>
                  <div style={{ padding: "16px 20px", borderBottom: "0.5px solid #e8eaf0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <h2 style={{ fontSize: 15, fontWeight: 600, color: "#dc2626", margin: 0 }}>⚠️ Infracciones jornada máxima 37.5h</h2>
                      <p style={{ fontSize: 12, color: "#a0aec0", margin: "2px 0 0" }}>Semanas con exceso de jornada según normativa 2026</p>
                    </div>
                    <button onClick={exportarPDF}
                      style={{ background: "#dc2626", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#fef2f2" }}>
                        {["Nº Emp.", "Nombre", "Apellidos", "Semana", "Horas trabajadas", "Exceso", "Gravedad"].map(h => (
                          <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#991b1b", borderBottom: "1px solid #fecaca" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {alertas.alertas.map((a: any, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 === 0 ? "#fff" : "#fff5f5" }}>
                          <td style={{ padding: "9px 14px", fontSize: 12, color: "#718096" }}>{a.numeroEmpleado}</td>
                          <td style={{ padding: "9px 14px", fontSize: 13, fontWeight: 500 }}>{a.nombre}</td>
                          <td style={{ padding: "9px 14px", fontSize: 13 }}>{a.apellidos}</td>
                          <td style={{ padding: "9px 14px", fontSize: 12 }}>{a.semana}</td>
                          <td style={{ padding: "9px 14px", fontSize: 13, fontWeight: 600, color: "#dc2626" }}>{a.horasTrabajadas}h</td>
                          <td style={{ padding: "9px 14px" }}>
                            <span style={{ background: "#fee2e2", color: "#dc2626", padding: "2px 8px", borderRadius: 20, fontSize: 12, fontWeight: 700 }}>+{a.horasExceso}h</span>
                          </td>
                          <td style={{ padding: "9px 14px" }}>
                            <span style={{ background: a.gravedad === "CRITICA" ? "#7f1d1d" : a.gravedad === "ALTA" ? "#dc2626" : "#f97316", color: "#fff", padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>
                              {a.gravedad}
                            </span>
                          </td>
                        </tr>
          ...
          <div style={{ marginTop: 12, padding: "12px 16px", background: "#fff", borderRadius: 12, border: "0.5px solid #e8eaf0" }}>
            <div style={{ fontSize: 11, color: "#a0aec0", display: "flex", gap: 24, flexWrap: "wrap" }}>
              ...
              <span>⚠️ Alertas jornada máxima 37.5h semanales</span>
