...
export default function ListaSolicitudesAccesoPublica() {
  ...
  const cargar = () => {
    setCargando(true)
    fetch("/api/solicitudes-acceso-publica").then(r => r.json()).then(d => {
  ...
  const confirmarAccion = async () => {
    if (!modalActivo || !masterPassword.trim()) { setError("Introduce tu contrasena master"); return }
    setProcesando(true)
    setError("")
    const res = await fetch("/api/solicitudes-acceso-publica", {
      method: "PATCH",
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#1e1b4b" }}>Solicitudes publicas de acceso</div>
      ...
      <div style={{ background: "#fff", border: "1px solid #BFDBFE", borderRadius: 10, overflow: "hidden" }}>
        {solicitudes.map((s, i) => (
          <div key={s.id} style={{ padding: "12px 14px", borderBottom: i < solicitudes.length - 1 ? "1px solid #f3f4f6" : "none", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, background: "#EFF6FF" }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 12.5, fontWeight: 600, color: "#1e1b4b" }}>{s.nombre}</div>
              <div style={{ fontSize: 10.5, color: "#1E40AF" }}>{s.email}{s.empresa ? " · " + s.empresa : ""}</div>
              {s.motivo && <div style={{ fontSize: 10.5, color: "#64748B", marginTop: 2 }}>{s.motivo}</div>}
            </div>
        ...
        <div onClick={() => { setModalActivo(null); setMasterPassword(""); setError("") }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, padding: 26, width: "100%", maxWidth: 360 }}>
            ...
            <div style={{ fontSize: 12.5, color: gris, marginBottom: 16 }}>
              {modalActivo.accion === "aprobar" ? "Se enviara una invitacion real por email para crear la cuenta." : "Se marcara como rechazada."} Introduce tu contrasena master para confirmar.
            </div>
            <input
              type="password"
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              ...
              <button
                ...
                {procesando ? "Procesando..." : (modalActivo.accion === "aprobar" ? "Confirmar aprobacion" : "Confirmar rechazo")}
              </button>
            </div>
          </div>
        </div>
