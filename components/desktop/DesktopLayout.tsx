  ...
  return (
    <div className={`flex h-screen overflow-hidden${isLight ? " bg-gray-50" : ""}`} style={{ background: isLight ? undefined : "#1E1B2E", '--sidebar-bg': sidebarBg, '--paleta-acento': paleta.acento, '--paleta-grad-inicio': paleta.gradInicio, '--paleta-grad-fin': paleta.gradFin, '--paleta-fondo': paleta.fondo, '--paleta-texto': paleta.texto, '--accent': paleta.acento, '--accent-hover': paleta.gradFin, '--accent-dim': paleta.acento + '1a', '--chat-propio': empresa?.chatColorPropio || paleta.acento, '--chat-ajeno': empresa?.chatColorAjeno || '#ffffff' } as React.CSSProperties}>
      <style>{`
        ...
        .nav-item { display:flex; align-items:center; gap:12px; height:48px; padding:0 20px; margin:0 auto 4px; width:98%; border-radius:3px; text-decoration:none; font-size:15px; font-weight:600; color:#1E1B4B; transition:all 0.2s ease; cursor:pointer; box-sizing:border-box; background:rgba(148,163,184,0.10); backdrop-filter:blur(5px); -webkit-backdrop-filter:blur(5px); box-shadow:0 2px 5px rgba(100,116,139,0.18); }
      ...
      <aside style={{ width: isMobile ? 320 : 332, background: 'transparent', display:'flex', flexDirection:'column', flexShrink:0, transition:'transform 0.25s', overflow:'hidden', padding: isMobile ? 0 : '12px 0 12px 3px', borderRadius: isMobile ? 0 : '16px 0 16px 16px', boxSizing: 'border-box',
          ...
          <div style={{ position:'absolute', inset:0, background: 'linear-gradient(180deg, color-mix(in srgb, var(--paleta-fondo) 40%, white), var(--paleta-fondo))', zIndex:0 }} />
          <svg style={{ position:"absolute", inset:0, zIndex:0, pointerEvents:"none" }} width="100%" height="100%" viewBox="0 0 360 1200" preserveAspectRatio="none"><defs><radialGradient id="sbGlow"><stop stopColor="var(--paleta-acento)" stopOpacity=".22"/><stop offset="1" stopColor="var(--paleta-acento)" stopOpacity="0"/></radialGradient></defs><ellipse cx="300" cy="220" rx="260" ry="300" fill="url(#sbGlow)" /><path d="M-30 390C80 320 160 500 390 400" stroke="var(--paleta-acento)" strokeOpacity=".2" strokeWidth="50" fill="none" /><path d="M-20 720C120 640 220 830 390 740" stroke="var(--paleta-acento)" strokeOpacity=".15" strokeWidth="44" fill="none" /></svg>
      ...
      {isMobile && (
        ...
        <header className={`flex items-center justify-between h-14 px-6 flex-shrink-0${isLight ? " bg-white border-b border-gray-200 shadow-sm" : ""}`}
          ...
          <div className="flex items-center gap-2">
            ...
            <img src="/design-system/sidebar/icons/scheduleo-logo-horizontal.png" alt="Scheduleo" style={{ height:32, width:"auto" }} />
          </div>
        </header>
        <BetaBanner />
        <main className={`flex-1 main-responsive-padding${isLight ? " bg-gray-50" : ""}`} style={{ background: fondoWorkspace ? undefined : "var(--bg)", position: "relative", padding: (pathname === "/chat" || pathname === "/panel-ejecutivo" || pathname === "/configuracion") ? 0 : 24, overflow: (pathname === "/chat" || pathname === "/configuracion") ? "hidden" : "auto", height: (pathname === "/chat" || pathname === "/configuracion") ? "100%" : "auto", display: "flex", flexDirection: "column" }}>
          {fondoWorkspace && (
            <>
              <div style={{ position: "fixed", top: 0, left: isMobile ? 0 : 332, right: 0, bottom: 0, backgroundImage: `url(${fondoWorkspace})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat", filter: `brightness(${fondoBrillo}%)`, zIndex: 0 }} />
              <div style={{ position: "fixed", top: 0, left: isMobile ? 0 : 332, right: 0, bottom: 0, background: `rgba(249,250,251,${fondoOpacidad / 100})`, zIndex: 0 }} />
            </>
          )}
          <style>{`
