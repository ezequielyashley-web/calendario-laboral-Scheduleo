...
export default function ChatDesktop() {
  ...
  return (
    <div style={{ display:"flex", height:"calc(100vh - 57px)", overflow:"hidden", position:"relative" as const }}>
      ...
      {/* SIDEBAR */}
      <div style={{ width: isMobile ? "100%" : sidebarWidth, background:"rgba(240,244,248,0.96)", backdropFilter:"blur(6px)", borderRight: isMobile ? "none" : "1px solid #dde3ea", display: isMobile && convActiva ? "none" : "flex", flexDirection:"column", flexShrink:0, position:"relative" as const, minWidth: isMobile ? "auto" : 220, maxWidth: isMobile ? "100%" : 480 }}>
      ...
      <div style={{ flex:1, display: isMobile && !convActiva ? "none" : "flex", minWidth:0, width: isMobile ? "100%" : "auto" }}>
        {convActiva ? (
          <>
            {/* Panel mensajes */}
            <div style={{ flex:1, display:"flex", flexDirection:"column", background:"rgba(229,221,213,0.96)", backgroundImage:"radial-gradient(rgba(0,0,0,0.035) 1px, transparent 1px)", backgroundSize:"18px 18px", minWidth:0 }}>
                ...
                {mensajes.length === 0 ? (
                  ...
                  return (
                    <div key={m.id} style={{ display:"flex", gap:6, maxWidth:"80%", alignSelf:esMio?"flex-end":"flex-start", flexDirection:esMio?"row-reverse":"row" }}>
                      ...
                      <div>
                        <div style={{ padding:"7px 11px", borderRadius:esMio?"12px 4px 12px 12px":"4px 12px 12px 12px", background:esMio?"var(--chat-propio)":"var(--chat-ajeno)", color:esMio?"#fff":"#111827", fontSize:12, lineHeight:1.5, boxShadow:"0 1px 2px rgba(0,0,0,0.1)" }}>{m.contenido}</div>
                        <div style={{ fontSize:10, color:"#9ca3af", marginTop:2, textAlign:esMio?"right":"left" }}>
                          ...
                          {esMio && <span style={{ color:"var(--chat-propio)", marginLeft:3 }}>✓✓</span>}
                        </div>
                      </div>
                    </div>
                  )
            ...
            {/* Panel info lateral */}
            <div style={{ width:220, borderLeft:"1px solid #dde3ea", background:"rgba(255,255,255,0.97)", backdropFilter:"blur(6px)", display: isMobile ? "none" : "flex", flexDirection:"column", flexShrink:0 }}>
          ...
          <>
            {/* Area vacia con logo */}
            <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"rgba(232,238,243,0.96)", gap:12 }}>
