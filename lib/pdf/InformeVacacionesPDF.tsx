  ...
  return (
    <Document>
      <Page size="A4" style={estilosBase.pagina}>
        <View style={estilosBase.headerBanda}>
          <Text style={estilosBase.logoTexto}>Scheduleo</Text>
          <View style={estilosBase.headerDerecha}>
            <Text style={estilosBase.confidencialPill}>CONFIDENCIAL</Text>
            <Text style={estilosBase.fechaGeneracion}>Generado el {fechaHoy()}</Text>
          </View>
        </View>
        <View style={estilosBase.contenido}>
          ...
          <View style={estilosBase.seccion}>
            ...
            <View style={estilosBase.tabla}>
              <View style={estilosBase.filaTablaHeader}>
                <Text style={estilosBase.celdaHeader}>Fecha inicio</Text>
                <Text style={estilosBase.celdaHeader}>Fecha fin</Text>
                <Text style={estilosBase.celdaHeader}>Estado</Text>
                <Text style={{ ...estilosBase.celdaHeader, flex: 2 }}>Motivo</Text>
              </View>
              {vacaciones.length === 0 && (
                <View style={estilosBase.filaTabla}><Text style={estilosBase.celda}>No hay registros</Text></View>
              )}
              {vacaciones.map((v, i) => (
                <View key={i} style={i % 2 === 1 ? estilosBase.filaTablaZebra : estilosBase.filaTabla}>
        ...
        <View style={estilosBase.footer} fixed>
          <Text style={estilosBase.footerTexto}>Documento generado automaticamente por Scheduleo 2.0</Text>
          <Text style={estilosBase.footerTexto} render={({ pageNumber, totalPages }) => `Pagina ${pageNumber} de ${totalPages}`} />
        </View>
      </Page>
    </Document>
  )
